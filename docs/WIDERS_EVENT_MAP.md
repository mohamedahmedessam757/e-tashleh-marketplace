...
> **Aug 2026 order/shipment:** Nest uses `{family}_ar_v3` only — **no URL button**; Nest injects absolute `follow_url` as body `{{4}}`. Retired `_ar_v2` order/shipment templates may be deleted in Meta.  
...
| `txn_order_customer` | `txn_order_customer_ar_v3` | `ORDER_CREATED`, `ORDER_STATUS`, `OFFER_REVEAL`, `OFFER_ACCEPTED` | `OFFER`, `ORDER`, `ORDER_UPDATE`, payment without invoice |
| `txn_order_merchant` | `txn_order_merchant_ar_v3` | same | Same as customer, merchant role |
| `txn_shipment_customer` | `txn_shipment_customer_ar_v3` | `SHIPMENT_STATUS` | `SHIPMENT_UPDATE` |
| `txn_shipment_merchant` | `txn_shipment_merchant_ar_v3` | `SHIPMENT_STATUS` | `SHIPMENT_UPDATE` |
...
`ORDER_CREATED` | `ORDER_STATUS` | `OFFER_REVEAL` | `OFFER_ACCEPTED` | `OFFER_BIDDING_RESTRICTED` | `VIOLATION_ISSUED` | `PAYMENT_SUCCESS` | `INVOICE_ISSUED` | `SHIPMENT_STATUS` | `WAYBILL_ISSUED` | `VERIFICATION` | `DOCUMENT` | `STORE_ACTIVATION` | `STORE_UNDER_REVIEW` | `STORE_PENDING_STRIPE` | `STORE_STRIPE_RESULT` | `CHAT_MESSAGE`
