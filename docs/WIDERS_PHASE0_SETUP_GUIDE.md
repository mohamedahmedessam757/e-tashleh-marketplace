...
| اسم تقني | Header | Body vars | زر URL |
|----------|--------|-----------|--------|
| `txn_order_customer_ar_v3` | `تحديث حالة الطلب` | 4 — `{{4}}` = follow_url | **لا** |
| `txn_order_merchant_ar_v3` | `تحديث حالة الطلب` | 4 — `{{4}}` = follow_url | **لا** |
| `txn_shipment_customer_ar_v3` | `تحديث الشحن` | 4 — `{{4}}` = follow_url | **لا** |
| `txn_shipment_merchant_ar_v3` | `تحديث شحن الطلب` | 4 — `{{4}}` = follow_url | **لا** |

في Widers «إعداد القالب» اربط المتغيرات بالترتيب:
...
| `{{2}}` | `order_number` |
| `{{3}}` | `status_detail` |
| `{{4}}` | `follow_url` (**ليس** اسم جهة الاتصال) |

Footer: `إي-تشليح | E-TASHLEH`

### Body — `txn_order_customer_ar_v3`
...
للتفاصيل افتح الرابط التالي على المنصة:
{{4}}

شكراً لاستخدامك منصتنا.
```

### Body — `txn_order_merchant_ar_v3`
...
### Body — `txn_shipment_customer_ar_v3`
...
لمتابعة الشحن على المنصة افتح الرابط:
{{4}}

شكراً لثقتك بنا.
```

### Body — `txn_shipment_merchant_ar_v3`
...
## قيم الرابط التي يرسلها Nest في `{{4}}`

| جمهور | طلب | شحن |
|-------|-----|-----|
| عميل | `https://e-tashleh.net/dashboard/order-details/{orderId}` | نفس المسار + `?tab=waybills` |
| تاجر | `https://e-tashleh.net/dashboard/explore-offer/{orderId}` | نفس المسار + `?tab=waybills` |
