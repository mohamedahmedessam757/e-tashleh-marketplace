...
import {
  ...
  type OrderFinancialTimelineData,
...
const DrawerBody = memo(function DrawerBody({
  ...
  if (loading && !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[240px] gap-4 opacity-50">
  ...
  if (!data) {
    return (
      <div className="flex items-center justify-center min-h-[200px] text-white/10 italic text-sm">
  ...
  return (
    <div className="flex flex-col h-full min-h-0 gap-6">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-col items-center gap-2 flex-1 min-w-0">
              ...
              <div className="text-center min-w-0">
                <div className="text-xs font-black text-white truncate max-w-[120px]">{data.customer.name}</div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-2 flex-1 min-w-0">
              <div className="flex -space-x-4 rtl:space-x-reverse">
                {data.merchants.slice(0, 3).map((m, idx) => (
                  <div
                    key={m.id}
      ...
      {(data.summary.totalRefunded > 0 || data.summary.hasDispute) && (
        <div className="shrink-0 p-4 rounded-2xl bg-rose-500/5 border border-rose-500/10 space-y-3">
          ...
          {data.summary.totalRefunded > 0 && (
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-mono font-black text-rose-400">
                -{Number(data.summary.totalRefunded).toLocaleString()} AED
              </span>
            </div>
          )}
        </div>
...
export const OrderFinancialDrawer: React.FC<OrderFinancialDrawerProps> = memo(function OrderFinancialDrawer({
  ...
  return createPortal(
    <>
      <div className="fixed inset-0 bg-black/70 z-[100]" onClick={onClose} aria-hidden="true" />

      <aside
        role="dialog"
        aria-modal="true"
        aria-label={t.admin.billing.ledger.auditDrawer.title}
        className={`fixed top-0 bottom-0 ${isAr ? 'left-0 border-r' : 'right-0 border-l'} w-full sm:max-w-xl bg-[#0A0908] border-white/10 z-[101] shadow-2xl flex flex-col [contain:layout_style_paint] [transform:translate3d(0,0,0)]`}
      >
        <div className="p-4 sm:p-6 border-b border-white/5 bg-white/[0.02] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-white/40 font-mono truncate">#{orderLabel}</p>
            </div>
          </div>
          <button
            type="button"
