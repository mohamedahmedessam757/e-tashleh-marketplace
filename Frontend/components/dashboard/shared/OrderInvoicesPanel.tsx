...
} from './invoices/InvoiceTypedDocument';
import {
    INVOICE_DOC_TABS,
    InvoiceDocTab,
    filterInvoicesByTab,
...
interface OrderInvoicesPanelProps {
    orderId: string;
    role: 'ADMIN' | 'SUPER_ADMIN' | 'MERCHANT' | 'CUSTOMER' | 'VERIFICATION_OFFICER' | 'ACCOUNTANT' | 'SUPPORT';
    initialData?: any[];
...
export const OrderInvoicesPanel: React.FC<OrderInvoicesPanelProps> = ({
    ...
    useEffect(() => {
        if (isSystemAdmin && initialDocTab) {
            setActiveDocTab(initialDocTab);
        }
    }, [initialDocTab, isSystemAdmin]);
    ...
    const tabLabel = (tab: InvoiceDocTab) => {
        const map: Record<InvoiceDocTab, string> = {
            MASTER: docs.tabMaster || (isAr ? 'شاملة' : 'Master'),
    ...
    const typedLabels = {
        ...
        issueDate: docs.issueDate || (isAr ? 'تاريخ الإصدار' : 'Issue Date'),
        partName: docs.partName || (isAr ? 'اسم القطعة' : 'Part name'),
        partPrice: docs.partPrice || (isAr ? 'قيمة القطعة' : 'Part price'),
        shippingCompany: docs.shippingCompany || (isAr ? 'شركة الشحن' : 'Shipping company'),
        shippingPending: docs.shippingPending || (isAr ? 'قيد التحديد' : 'Pending assignment'),
        shippingRevenue: docs.shippingRevenue || (isAr ? 'إيراد الشحن' : 'Shipping revenue'),
        ...
        emptyTitle: docs.emptyTitle || (isAr ? 'لا توجد فواتير لهذا النوع' : 'No invoices for this type'),
        emptyHint: docs.emptyHint || (isAr ? 'قد لا توجد قيمة شحن أو لم تُنشأ الوثيقة بعد.' : 'There may be no shipping amount, or the document is not created yet.'),
        groupId: docs.groupId || (isAr ? 'مجموعة الفواتير' : 'Invoice group'),
    };
                        ...
                        <div className="flex flex-wrap gap-2 no-print">
                            {INVOICE_DOC_TABS.map((tab) => {
                                ...
                                return (
                                    ...
                                    >
                                        {tabLabel(tab)}
                                        <span className={`ms-2 font-mono ${active ? 'text-black/60' : 'text-white/30'}`}>
                                            {count}
                                        </span>
                                    </button>
                                );
