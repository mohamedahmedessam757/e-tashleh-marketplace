...
const USAGE_CONDITIONS = [
    { id: 'UNUSED' as const, labelAr: 'لم يتم الاستخدام', labelEn: 'Unused' },
    { id: 'OPENED' as const, labelAr: 'مفتوح الغلاف', labelEn: 'Packaging Opened' },
    { id: 'INSTALLED' as const, labelAr: 'تم تركيب القطعة', labelEn: 'Item Installed' },
];
...
export const ReturnRequestModal: React.FC<ReturnRequestModalProps> = ({ 
    ...
    // Lock body scroll while open — avoids double-scroll jank with page behind
    useEffect(() => {
        if (!isOpen) return;
    ...
    const handleConfirmationChange = useCallback((key: 'integrity' | 'packaging' | 'policy', checked: boolean) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 overflow-hidden [contain:layout]">
            ...
            <div
                role="dialog"
                aria-modal="true"
                className="w-full max-w-lg relative z-10 [contain:layout_paint]"
            >
                <div className="p-0 border border-white/10 rounded-[2rem] overflow-hidden shadow-2xl bg-[#0A0A0A] relative flex flex-col max-h-[min(92vh,900px)]">
                    ...
                    <div
                        ref={scrollRef}
                        className="p-6 space-y-6 flex-1 min-h-0 overflow-y-auto overscroll-contain touch-pan-y custom-scrollbar [contain:content] [-webkit-overflow-scrolling:touch]"
                    >
                        {loadingParts ? (
                        ...
                        <div className="flex items-center gap-6 p-6 bg-white/[0.02] border border-white/5 rounded-3xl">
                            ...
                            <div className="space-y-1 min-w-0">
                                ...
                                <h4 className="text-xl font-black text-white uppercase tracking-tight truncate">{activePartName}</h4>
                                <p className="text-xs font-bold text-white/40 flex items-center gap-2 uppercase truncate">
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    {USAGE_CONDITIONS.map((cond) => (
                                        <button
                                            key={cond.id}
                                            type="button"
                                            onClick={() => setUsageCondition(cond.id)}
                                            className={`
                                                p-4 rounded-2xl border transition-colors text-center
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className={`w-full bg-[#0A0A0A] border rounded-2xl px-5 py-4 text-xs text-white outline-none transition-colors resize-none ${isAr ? 'text-right' : 'text-left'}
                    ...
                    <div className="p-6 border-t border-white/5 bg-black/40 space-y-3 shrink-0">
                        ...
                        <button
                            ...
                            className="w-full py-4 bg-gradient-to-r from-cyan-600 to-cyan-500 hover:from-cyan-500 hover:to-cyan-400 text-white rounded-2xl font-black uppercase tracking-widest text-sm transition-colors flex items-center justify-center gap-3 disabled:opacity-50 shadow-lg shadow-cyan-600/20"
                        >
                            {isSubmitting ? (
                                <Loader2 className="animate-spin" size={20} />
                            ) : (
